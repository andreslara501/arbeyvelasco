$(document).ready(function() {
    ...
    $("body").on("click", ".editar", function(){
        ...
        $("#editar [name=titulo]").val($(this).parent().parent().attr("titulo"));
    ...
    $("#formulario_editar_item").submit(function(e){
        ...
        .done(function(data){
            ...
            $("#" + $("#editar").attr("id_elemento")).attr("titulo", $("#editar [name=titulo]").val());
            $("#" + $("#editar").attr("id_elemento") + " img").attr("src", "../../uploads/links/" + $("#editar").attr("id_elemento") + ".jpg?random" + Math.round(Math.random()*10));
    ...
    $('#eliminar .si').on('click', function() {
        ...
        $.ajax({
            url: "/api/links/eliminar/" + id_elemento + "/",
            type: "post",
            dataType: "json"
        })
        .done(function(data){
    ...
    $("#formulario_nuevo_item").submit(function(e){
        ...
        .done(function(data){
            $("#sortable").append("\
                <li id=\""+data["respuesta"]+"\" class=\"ui-state-default clearfix\" orden=\"" + data["respuesta"] + "\" descripcion=\"" + $("[name=descripcion]").val() + "\" enlace=\"" + $("[name=enlace]").val() + "\" titulo=\"" + $("[name=titulo]").val() + "\">\
                    <div class=\"column large-2\">\
                        <img src=\"../../uploads/links/"+data["respuesta"]+".jpg\">\
                    </div>\
