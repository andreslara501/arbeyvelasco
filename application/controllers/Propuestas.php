...
class Propuestas extends CI_Controller {
	...
	public function index(){
		...
		$data_propuestas = $this 	-> db
							 			-> order_by("id","DESC")
							 			-> where("pagina = 8")
							 			-> limit(40)
							 			-> get('articulos');
		$data_propuestas = $data_propuestas -> result_array();
		...
		$data["propuestas"] = $data_propuestas;
		...
		$this -> theme_load($data, 'propuestas');
	}

	public function propuesta($alias){
		/* data Menú */
		$data_menu = $this 	-> db
							-> order_by("orden", "asc")
							-> get('menu');
		...
		/* data Ultimas noticias */
		$data_propuesta  = $this 	-> db
							 			-> order_by("id","DESC")
							 			-> where("pagina = 8 AND alias LIKE '" . $alias . "'")
							 			-> get('articulos');
		$data_propuesta = $data_propuesta -> row_array();
		...
		$data["menus"] = $data_menu -> result();
		$data["paginas"] = $data_paginas;
		$data["propuesta"] = $data_propuesta;
		$data["informacion_principal"] = $this -> noticias_seccion("informacion-principal", 2);
		$data["noticias"] = $this -> noticias_seccion("noticias", 20);
		$data["publicidades"] = $data_publicidades;
		$data["etiquetas"] = $data_etiquetas;
		$this -> theme_load($data, 'propuesta');
	}
